...
static void GetPalaceData(void)
{
    ...
    case PALACE_DATA_PRIZE:
        gSpecialVar_Result = gSaveBlock1Ptr->frontier.palacePrize;
        break;
    case PALACE_DATA_WIN_STREAK:
        gSpecialVar_Result = gSaveBlock1Ptr->frontier.palaceWinStreaks[battleMode][lvlMode];
        break;
...
static void SetPalaceData(void)
{
    ...
    {
    case PALACE_DATA_PRIZE:
        gSaveBlock1Ptr->frontier.palacePrize = gSpecialVar_0x8006;
        break;
    case PALACE_DATA_WIN_STREAK:
        gSaveBlock1Ptr->frontier.palaceWinStreaks[battleMode][lvlMode] = gSpecialVar_0x8006;
        break;
    case PALACE_DATA_WIN_STREAK_ACTIVE:
...
static void GetPalaceCommentId(void)
{
    ...
    if (gSaveBlock1Ptr->frontier.palaceWinStreaks[battleMode][lvlMode] < 50)
        gSpecialVar_Result = Random() % 3;
    else if (gSaveBlock1Ptr->frontier.palaceWinStreaks[battleMode][lvlMode] < 99)
...
static void IncrementPalaceStreak(void)
{
    ...
    if (gSaveBlock1Ptr->frontier.palaceWinStreaks[battleMode][lvlMode] < MAX_STREAK)
    {
        ...
        if (gSaveBlock1Ptr->frontier.palaceWinStreaks[battleMode][(lvlMode > gSaveBlock1Ptr->frontier.palaceRecordWinStreaks[battleMode][lvlMode]) ? 1 : 0])
...
static void SetRandomPalacePrize(void)
{
    ...
    if (gSaveBlock1Ptr->frontier.palaceWinStreaks[battleMode][lvlMode] > 41)
        gSaveBlock1Ptr->frontier.palacePrize = sBattlePalaceLatePrizes[Random() % ARRAY_COUNT(sBattlePalaceLatePrizes)];
    else
        gSaveBlock1Ptr->frontier.palacePrize = sBattlePalaceEarlyPrizes[Random() % ARRAY_COUNT(sBattlePalaceEarlyPrizes)];
}

static void GivePalacePrize(void)
{
    if (AddBagItem(gSaveBlock1Ptr->frontier.palacePrize, 1) == TRUE)
    {
        CopyItemName(gSaveBlock1Ptr->frontier.palacePrize, gStringVar1);
        gSaveBlock1Ptr->frontier.palacePrize = 0;
