...
#define ADDITIONAL_FLAGS_IF_HARDMODE (AI_FLAG_TRY_TO_FAINT | AI_FLAG_CHECK_VIABILITY | AI_FLAG_SMART_MON_CHOICES | AI_FLAG_SMART_SWITCHING)
...
void BattleAI_SetupFlags(void)
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_RECORDED)
        ...
        AI_THINKING_STRUCT->aiFlags = AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_SMART_MON_CHOICES | AI_FLAG_SMART_SWITCHING;
    else if (gBattleTypeFlags & BATTLE_TYPE_TWO_OPPONENTS) {
        if (FlagGet(FLAG_SYS_ENFORCED_HARDMODE))
            AI_THINKING_STRUCT->aiFlags = (gTrainers[gTrainerBattleOpponent_A].aiFlags | gTrainers[gTrainerBattleOpponent_B].aiFlags) | ADDITIONAL_FLAGS_IF_HARDMODE;
        else
            AI_THINKING_STRUCT->aiFlags = gTrainers[gTrainerBattleOpponent_A].aiFlags | gTrainers[gTrainerBattleOpponent_B].aiFlags;
    }
    else {
        if (FlagGet(FLAG_SYS_ENFORCED_HARDMODE))
            AI_THINKING_STRUCT->aiFlags = gTrainers[gTrainerBattleOpponent_A].aiFlags | ADDITIONAL_FLAGS_IF_HARDMODE;
        else
...
#undef ADDITIONAL_FLAGS_IF_HARDMODE
