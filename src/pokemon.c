...
u16 GetEvolutionTargetSpecies(struct Pokemon *mon, u8 mode, u16 evolutionItem, struct Pokemon *tradePartner)
{
    ...
    case EVO_MODE_NORMAL:
        ...
        for (i = 0; i < EVOS_PER_MON; i++)
        {
            ...
            case EVO_FRIENDSHIP_DAY:
                RtcCalcLocalTime();
                if (gLocalTime.hours >= 8 && gLocalTime.hours < 20 && friendship >= 220)
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
                break;
            case EVO_LEVEL_DAY:
                RtcCalcLocalTime();
                if (gLocalTime.hours >= 8 && gLocalTime.hours < 20 && gEvolutionTable[species][i].param <= level)
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
                break;
            ...
            case EVO_ITEM_HOLD_DAY:
                RtcCalcLocalTime();
                if (gLocalTime.hours >= 8 && gLocalTime.hours < 20 && heldItem == gEvolutionTable[species][i].param)
                {
                    heldItem = 0;
                    SetMonData(mon, MON_DATA_HELD_ITEM, &heldItem);
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
            ...
            case EVO_LEVEL_DUSK:
                RtcCalcLocalTime();
                if (gLocalTime.hours >= 18 && gLocalTime.hours < 20 && gEvolutionTable[species][i].param <= level)
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
                break;
            case EVO_LEVEL:
    ...
    case EVO_MODE_ITEM_CHECK:
        for (i = 0; i < EVOS_PER_MON; i++)
        {
            ...
            case EVO_ITEM_NIGHT:
                RtcCalcLocalTime();
                if (gLocalTime.hours >= 20 && gLocalTime.hours < 8 && gEvolutionTable[species][i].param == evolutionItem)
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
                break;
            case EVO_ITEM_DAY:
                RtcCalcLocalTime();
                if (gLocalTime.hours >= 8 && gLocalTime.hours < 20 && gEvolutionTable[species][i].param == evolutionItem)
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
                break;
            }
        }
            ...
            if (method == formChanges[i].method && species != formChanges[i].targetSpecies)
            {
                ...
                case FORM_CHANGE_ITEM_USE:
                    if (arg == formChanges[i].param1)
                    {
                        ...
                        case DAY:
                            RtcCalcLocalTime();
                            if (gLocalTime.hours >= 8 && gLocalTime.hours < 20)
                                targetSpecies = formChanges[i].targetSpecies;
                            break;
                        case NIGHT:
                            RtcCalcLocalTime();
                            if (gLocalTime.hours >= 20 && gLocalTime.hours < 8)
                                targetSpecies = formChanges[i].targetSpecies;
                            break;
                        default:
                            targetSpecies = formChanges[i].targetSpecies;
